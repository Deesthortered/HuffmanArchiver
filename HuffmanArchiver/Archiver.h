...
namespace spaceArchiver
{
	...
	public:
		...
		void Run()
		{
			...
			fout.write((char*)&max_byte_size, sizeof(unsigned char));			// unsigned char    - 1 byte // количество байт дл€ кода максимальной длины
			
			char *buff = new char[max_byte_size];
			end_bs.GetMemory(buff);
			//«апись метки конца файла в таблицу
	...
	public:
		...
		void Run()
		{
			...
			unsigned char max_byte_size;	// количество байт дл€ кода максимальной длины
